...
let ticketPriceTotal = runners.reduce((acc, money) => acc += money.donation, 0);
...
// Problem 1
// Companies want to spotlight major donors. The organizers need to know who donated more than $200.
let bigDonors = runners.filter((money) => money.donation > 200)
console.log(bigDonors)
// Problem 2
//The organizers need a master list of emails to send out all communications.
let emailList = runners.map((address) => address.email)
console.log(emailList)
// Problem 3
//The organizers have decided that the race will begin in alphabetical order based on last name.
let raceOrder = runners.map((name) => name.last_name)
console.log(raceOrder.sort())